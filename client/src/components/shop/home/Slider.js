  ...
  return (
    <Fragment>
      <div className="relative mt-16 overflow-hidden bg-gray-900" style={{ minHeight: "520px" }}>
        ...
        {/* Hero content */}
        <div className="relative z-10 flex flex-col justify-center h-full px-8 md:px-20 py-28 md:py-36 max-w-3xl">
          <AnimatePresence mode="wait">
            <motion.div key={slide} className="flex flex-col">
              <motion.span
                custom={0}
              ...
              <motion.h1
                ...
                initial="hidden"
                animate="visible"
                className="text-4xl md:text-6xl font-bold text-white leading-tight mb-5 whitespace-pre-line"
              >
        ...
        {/* Prev / Next */}
        <button
          onClick={() => { setDirection(-1); setSlide((slide - 1 + STATIC_SLIDES.length) % STATIC_SLIDES.length); }}
          className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/10 hover:bg-white/25 border border-white/20 text-white rounded-full p-3 z-10 transition-all duration-200 backdrop-blur-sm"
        >
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7"/></svg>
        </button>
        <button
          onClick={() => { setDirection(1); setSlide((slide + 1) % STATIC_SLIDES.length); }}
          className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/10 hover:bg-white/25 border border-white/20 text-white rounded-full p-3 z-10 transition-all duration-200 backdrop-blur-sm"
        >
